...
//   const response = await fetch(`${LOCALHOST_DOMAIN}/profile/${requestData.profileId}`);
//   const data = await response.json();
...
export async function editProfile(requestData) {
  const response = await fetch(
    `${LOCALHOST_DOMAIN}/user/profile/update`,
    {
      method: "PUT",
      body: requestData.data,
      headers: {
...
// export async function getDraftBlogs(token) {
//   const response = await fetch(`${LOCALHOST_DOMAIN}/feed/blogs/drafts`, {
//     headers: {
//       Authorization: "Bearer " + token,
//     },
//   });
